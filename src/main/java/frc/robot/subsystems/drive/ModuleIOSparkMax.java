...
public class ModuleIOSparkMax implements ModuleIO {
  private static final double DRIVE_GEAR_RATIO =
      (45.0 * 22) / (14 /* REV Swerve Pinion teeth */ * 15);
  ...
  private final double turnAbsoluteEncoder;
  ...
  public ModuleIOSparkMax(int index) {
    switch (index) {
      case 0: // Rear left
        ...
        turnAbsoluteEncoder = turnSparkMax.getAbsoluteEncoder().getPosition();
        absoluteEncoderOffset = new Rotation2d(0.01); // MUST BE CALIBRATED
        break;
      case 1: // Front left
        driveSparkMax = new CANSparkMax(4, MotorType.kBrushless);
        turnSparkMax = new CANSparkMax(8, MotorType.kBrushless);
        turnAbsoluteEncoder = turnSparkMax.getAbsoluteEncoder().getPosition();
        absoluteEncoderOffset = new Rotation2d(.9); // MUST BE CALIBRATED
        break;
      case 2: // Front right
        driveSparkMax = new CANSparkMax(6, MotorType.kBrushless);
        turnSparkMax = new CANSparkMax(13, MotorType.kBrushless);
        turnAbsoluteEncoder = turnSparkMax.getAbsoluteEncoder().getPosition();
        absoluteEncoderOffset = new Rotation2d(0.045); // MUST BE CALIBRATED
        break;
      case 3: // Rear Right
        driveSparkMax = new CANSparkMax(3, MotorType.kBrushless);
        turnSparkMax = new CANSparkMax(5, MotorType.kBrushless);
        turnAbsoluteEncoder = turnSparkMax.getAbsoluteEncoder().getPosition();
        absoluteEncoderOffset = new Rotation2d(0.331); // MUST BE CALIBRATED
        break;
      default:
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    inputs.turnAbsolutePosition =
        new Rotation2d(turnAbsoluteEncoder / RobotController.getVoltage5V() * 2.0 * Math.PI)
